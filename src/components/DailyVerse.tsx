...
const DailyVerse: React.FC = () => {
  ...
  useEffect(() => {
    const fetchVerse = async () => {
      ...
      try {
        ...
        setError("Não foi possível carregar o verso do dia.");
      }
  ...
  const copyToClipboard = async () => {
    if (!verse) return;
    const shareText = `"${verse.text}" - ${verse.reference} | Praise FM BRA`;
  ...
  const handleShare = async () => {
    if (!verse) return;
    const shareData = {
      title: 'Praise FM BRA - Versículo do Dia',
  ...
  if (loading) return (
    <div className="flex flex-col items-center justify-center py-20 text-gray-400">
      ...
      <p className="font-normal uppercase tracking-[0.2em] text-[10px]">Carregando Mensagem...</p>
    </div>
  );
  ...
  if (error) return (
    <div className="max-w-4xl mx-auto px-4 py-12 text-center">
      <div className="bg-red-50 dark:bg-red-900/10 p-10 border border-red-100 dark:border-red-900/20">
        ...
        <p className="text-red-600 dark:text-red-400 text-sm font-normal uppercase tracking-widest">{error}</p>
      </div>
    </div>
  );
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-12">
      <div className="bg-white dark:bg-[#111] shadow-xl overflow-hidden border border-gray-100 dark:border-white/5 flex flex-col md:flex-row transition-colors">
        <div className="md:w-1/3 bg-black relative p-10 flex flex-col justify-between overflow-hidden">
          ...
          <div className="relative z-10 mt-12 flex items-center space-x-3">
            ...
            <p className="text-gray-500 text-[9px] font-normal uppercase tracking-widest">{verse.translation_name}</p>
          </div>
        </div>
        <div className="md:w-2/3 p-8 md:p-12 flex flex-col">
          ...
          <div className="mt-auto pt-8 border-t border-gray-50 dark:border-white/5 flex flex-col sm:flex-row sm:items-center justify-between gap-6">
            <div>
              ...
              <p className="text-gray-400 text-[9px] font-normal uppercase tracking-widest mt-2">SAGRADAS ESCRITURAS</p>
            </div>
