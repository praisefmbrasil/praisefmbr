...
const LivePlayerBar: React.FC<LivePlayerBarProps> = ({ 
  ...
  const skip30Forward = () => {
    console.log('Avançar 30s - não disponível para streams ao vivo');
  };

  const skip30Backward = () => {
    console.log('Retroceder 30s - não disponível para streams ao vivo');
  };
  ...
  return (
    <>
      {/* AGENDA LATERAL - Só AO VIVO + próximos 4 */}
      <div 
          ...
          {!isExpanded ? (
            ...
            <div className="flex flex-col">
              <div className="flex items-center justify-between px-5 py-3 border-b border-gray-200 dark:border-white/10">
                <span className="text-sm font-semibold text-black dark:text-white">Tocando Agora</span>
                <button 
                  onClick={() => {
                    setIsExpanded(false);
