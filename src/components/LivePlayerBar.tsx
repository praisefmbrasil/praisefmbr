...
const LivePlayerBar: React.FC<LivePlayerBarProps> = ({ isPlaying, onTogglePlayback, program, liveMetadata, queue = [], audioRef }) => {
  ...
  const skip30Forward = () => {
    console.log('Avançar 30s - indisponível para transmissões ao vivo');
  };

  const skip30Backward = () => {
    console.log('Retroceder 30s - indisponível para transmissões ao vivo');
  };
        ...
        <div className="flex-grow overflow-y-auto pb-20 md:pb-0">
          ...
          <div className="p-4 border-b border-gray-100 dark:border-white/5 bg-gray-50 dark:bg-white/5">
            <div className="flex items-center mb-2">
              <span className="text-[10px] font-bold text-[#00d9c9] uppercase tracking-widest bg-[#00d9c9]/10 px-2 py-0.5 rounded">No Ar Agora</span>
            </div>
          ...
          {/* Próximos programas */}
          <div className="px-4 py-3 bg-gray-100 dark:bg-white/5 border-b border-gray-200 dark:border-white/10">
        ...
        >
          {!isExpanded ? (
            ...
            <div className="flex flex-col">
              ...
              {/* Restante do player mobile traduzido... */}
            </div>
          )}
        </div>
      )}
        ...
        <div className="fixed bottom-0 left-0 right-0 z-[60] bg-white dark:bg-[#121212] border-t border-gray-200 dark:border-white/10 hidden md:flex flex-col transition-colors duration-300">
          ...
          <div className="flex items-center justify-between px-8 py-4">
            ...
            <div className="flex items-center justify-center space-x-6">
              <button onClick={skip30Backward} className="relative w-10 h-10 flex items-center justify-center text-gray-700 dark:text-gray-300 hover:text-black dark:hover:text-white transition-colors">
                <RotateCcw className="w-5 h-5" strokeWidth={2} />
                <span className="absolute text-[9px] font-bold mt-[2px]">30</span>
              </button>

              <button onClick={onTogglePlayback} className="w-12 h-12 bg-black dark:bg-white text-white dark:text-black rounded-full flex items-center justify-center hover:scale-105 transition-all active:scale-95 shadow-md">
                {isPlaying ? <Pause className="w-5 h-5 fill-current" /> : <Play className="w-5 h-5 fill-current ml-0.5" />}
              </button>

              <button onClick={skip30Forward} className="relative w-10 h-10 flex items-center justify-center text-gray-700 dark:text-gray-300 hover:text-black dark:hover:text-white transition-colors">
            ...
            <div className="flex items-center justify-end space-x-4 w-[30%]">
              <div className="flex items-center space-x-2 relative" onMouseEnter={() => setShowVolumeSlider(true)} onMouseLeave={() => setShowVolumeSlider(false)}>
                ...
                <div className={`flex items-center transition-all duration-200 overflow-hidden ${showVolumeSlider ? 'w-24 opacity-100' : 'w-0 opacity-0'}`}>
                  <input type="range" min="0" max="1" step="0.01" value={isMuted ? 0 : volume} onChange={handleVolumeChange} className="w-full h-1 bg-gray-200 dark:bg-white/20 rounded-lg appearance-none cursor-pointer accent-black dark:accent-white" />
                  <span className="ml-2 text-xs font-medium text-gray-600 dark:text-gray-400 w-6 text-right">{Math.round((isMuted ? 0 : volume) * 10)}</span>
                </div>
              </div>

              <button onClick={cyclePlaybackRate} className="px-3 py-1.5 text-xs font-semibold text-gray-700 dark:text-gray-300 hover:text-black dark:hover:text-white border border-gray-300 dark:border-white/20 rounded hover:border-black dark:hover:border-white transition-all">
                {playbackRate}×
              </button>

              <button onClick={() => setShowSchedule(true)} className="p-2 text-gray-700 dark:text-gray-300 hover:text-black dark:hover:text-white transition-colors">
