...
const LivePlayerBar: React.FC<LivePlayerBarProps> = ({ isPlaying, onTogglePlayback, program, liveMetadata, queue = [], audioRef }) => {
  ...
  const skip30Backward = () => {
    console.log('Voltar 30s - não disponível para transmissões ao vivo');
  };
      ...
      >
        ...
        <div className="flex-grow overflow-y-auto pb-20 md:pb-0">
          ...
          <div className="p-4 border-b border-gray-100 dark:border-white/5 bg-gray-50/50 dark:bg-white/5">
            <span className="text-[10px] font-bold text-[#ff6600] uppercase tracking-widest mb-3 block">No Ar</span>
          ...
          <div className="px-4 py-3">
             <span className="text-[10px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-widest block">A Seguir</span>
          </div>
          {queue && queue.slice(0, 4).map((prog) => (
            <div key={prog.id} className="p-4 border-b border-gray-100 dark:border-white/5 hover:bg-gray-50 dark:hover:bg-white/5 transition-colors cursor-pointer">
              <div className="flex items-start space-x-3">
                <div className="w-16 h-16 flex-shrink-0 rounded overflow-hidden">
                  <img src={prog.image} className="w-full h-full object-cover" alt="" />
                </div>
                <div className="flex flex-col min-w-0 flex-grow">
                  <span className="font-bold text-sm text-black dark:text-white leading-tight mb-1">
                    {prog.title}
                  </span>
          ...
          {!isExpanded ? (
            ...
            <div className="flex flex-col">
              <div className="flex items-center justify-between px-5 py-3 border-b border-gray-200 dark:border-white/10">
                <span className="text-sm font-semibold text-black dark:text-white uppercase tracking-widest">Ouvindo Agora</span>
                <button 
              ...
              <div className="flex items-center space-x-4 px-5 py-6 border-b border-gray-100 dark:border-white/5">
                <div className="w-20 h-20 flex-shrink-0 rounded shadow-lg">
                  <img src={program.image} className="w-full h-full object-cover" alt="" />
                </div>
                <div className="flex flex-col min-w-0 flex-grow">
              ...
              {/* Barra de Progresso Estética (Live) */}
              ...
              <div className="flex items-center justify-center space-x-8 px-5 py-6">
                <button onClick={skip30Backward} className="relative w-12 h-12 flex items-center justify-center text-gray-700 dark:text-gray-300">
                ...
                <button onClick={skip30Forward} className="relative w-12 h-12 flex items-center justify-center text-gray-700 dark:text-gray-300">
              ...
              <button 
                ...
                className="w-full py-4 bg-gray-50 dark:bg-white/5 text-sm font-bold uppercase tracking-widest text-gray-600 dark:text-gray-400"
              >
                Ver Programação
              </button>
            </div>
