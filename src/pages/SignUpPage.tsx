...
const SignUpPage: React.FC = () => {
  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    if (error) {
      ...
      navigate('/login?message=Conta criada! Por favor, entre.');
    }
  };
    ...
    <div className="min-h-screen bg-[#f3f3f3] dark:bg-[#121212] flex items-center justify-center p-4">
      <div className="w-full max-w-md bg-white dark:bg-[#1a1a1a] rounded-2xl shadow-xl p-8 transition-colors">
        <div className="text-center mb-8">
          ...
          />
          <h1 className="text-3xl font-medium text-gray-900 dark:text-white tracking-tighter uppercase">Junte-se a nós</h1>
          <p className="text-gray-500 dark:text-gray-400 font-normal text-sm mt-2 uppercase">Comece sua jornada de adoração hoje</p>
        </div>
        ...
        <form onSubmit={handleSignUp} className="space-y-5">
          <div>
            <label className="block text-[10px] font-medium text-gray-400 uppercase tracking-widest mb-2">Endereço de e-mail</label>
            <input
              type="email"
              value={email}
              onChange={(e) => setEmail(e.target.value)}
              className="w-full px-5 py-4 bg-gray-50 dark:bg-black/20 border border-gray-100 dark:border-white/5 rounded-xl focus:ring-2 focus:ring-[#ff6600] outline-none dark:text-white font-normal"
          ...
          <div>
            <label className="block text-[10px] font-medium text-gray-400 uppercase tracking-widest mb-2">Confirmar senha</label>
            <input
              type="password"
