...
const MySoundsPage: React.FC = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-16">
      {/* Header com Estética Minimalista */}
      <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 border-b-2 border-black dark:border-white pb-6">
        <div>
          <div className="flex items-center space-x-3 text-[#ff6600] mb-2 font-medium uppercase tracking-[0.3em] text-[10px]">
            <Heart className="w-4 h-4 fill-current" />
            <span>Biblioteca Pessoal</span>
          </div>
          <h1 className="text-5xl md:text-7xl font-medium text-gray-900 dark:text-white tracking-tighter uppercase leading-[0.8]">
            Meus<br />Sons
          </h1>
        </div>
        <p className="text-gray-400 dark:text-gray-500 font-normal uppercase tracking-widest text-[10px] mt-4 md:mt-0">
      ...
      {favorites.length === 0 ? (
        ...
        <div className="text-center py-32 bg-gray-50 dark:bg-white/5 rounded-3xl border-2 border-dashed border-gray-200 dark:border-white/10">
          ...
          <h3 className="text-2xl font-medium text-gray-400 dark:text-gray-600 uppercase tracking-tight">Sua biblioteca está vazia</h3>
          <p className="text-gray-500 dark:text-gray-400 font-normal uppercase tracking-widest text-[10px] mt-4 max-w-xs mx-auto leading-relaxed">
            Clique no ícone de coração nas músicas e devocionais para salvá-los aqui e ouvir quando quiser.
          </p>
        </div>
      ) : (
        /* Grid de Cards */
            ...
            <div key={fav.id} className="group relative bg-white dark:bg-[#1a1a1a] rounded-2xl overflow-hidden shadow-lg border border-gray-100 dark:border-white/5 transition-all duration-300 hover:shadow-2xl">
              <div className="relative aspect-square overflow-hidden">
                ...
                <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                  <button className="w-16 h-16 bg-[#ff6600] rounded-full flex items-center justify-center text-white shadow-2xl transform scale-75 group-hover:scale-100 transition-all active:scale-90">
                    <Play className="w-8 h-8 fill-current ml-1" />
                  </button>
                </div>
                <div className="absolute top-4 left-4">
                  <span className="bg-black/80 backdrop-blur-md text-[#ff6600] text-[9px] font-black uppercase px-2 py-1 rounded-sm flex items-center space-x-1 shadow-lg border border-white/10">
